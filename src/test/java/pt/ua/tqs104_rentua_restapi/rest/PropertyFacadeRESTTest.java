...
@RunAsClient
public class PropertyFacadeRESTTest {
    ...
    /**
     * Test of find method, of class PropertyFacadeREST.
     */
    @Test
    @InSequence(4)
    public void testFind() throws Exception {
        HttpClient client = HttpClients.createDefault();
        HttpGet request = new HttpGet(UriBuilder.fromUri(baseURL).path("rest/property/"+propertyId).build().toString());
        HttpResponse response = client.execute(request);
        assertEquals(Response.Status.OK.getStatusCode(), response.getStatusLine().getStatusCode());
    }

    /**
     * Test of findAll method, of class PropertyFacadeREST.
     */
    @Test
    public void testFindAll() throws Exception {
        HttpClient client = HttpClients.createDefault();
        HttpGet request = new HttpGet(UriBuilder.fromUri(baseURL).path("rest/property").build().toString());
        HttpResponse response = client.execute(request);
        assertEquals(Response.Status.OK.getStatusCode(), response.getStatusLine().getStatusCode());
    }

    /**
     * Test of findByUser method, of class PropertyFacadeREST.
     */
    @Test
    @InSequence(5)
    public void testFindByUser() throws Exception {
        HttpClient client = HttpClients.createDefault();
        HttpGet request = new HttpGet(UriBuilder.fromUri(baseURL).path("rest/property/user/ze").build().toString());
        HttpResponse response = client.execute(request);
        assertEquals(Response.Status.OK.getStatusCode(), response.getStatusLine().getStatusCode());
    }
